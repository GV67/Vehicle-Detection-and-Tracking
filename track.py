...
if str(ROOT / 'best_yolov7') not in sys.path:
        ...
        for i, det in enumerate(pred):  # detections per image
                    ...
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox to image
                            ...
                            label = None if hide_labels else (f'{id} {names[c]}' if hide_conf else \
                                (f'{names[c]}' if hide_class else f'{id} {names[c]} {conf:.2f}'))
                            plot_one_box(bboxes, im0, label=label, color=colors[int(cls)], line_thickness=3)
                            if save_crop:
            ...
            if count:
                ...
                if save_txt:
                    ...
                    cv2.rectangle(im0, (x1, y1 + 0), (txt_size[0] * 3, y2+40),(0, 0, 0),-1) 
                    cv2.rectangle(im0, (x1, y1 + 41), (txt_size[0] * 3, y2+40),(0, 0, 0),-1) 
                    cv2.putText(im0, '--COUNTER--', (x1 + 1, y1 + 41), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 3)
                    cv2.putText(im0, '{}'.format(itemDict), (x1 + 1, y1 + 81), cv2.FONT_HERSHEY_SIMPLEX,1, (255, 255, 255), 3)
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', nargs='+', type=str, default=WEIGHTS / 'best_yolov7.pt', help='model.pt path(s)')
